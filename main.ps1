...
class Question {
    [int]$id
    [string]$type = 'default' # Default = text answer; code = answer generated by a scriptblock; setup = some preperations are made before the question is asked.
    [string]$prompt
...
$questions = @(
    [Question]::new(0, 'code', "Demo Question:`nWhat is the Process ID of the lsass.exe process? ask for a hint to reveal how to get the answer", { $(Get-Process lsass).Id.ToString() }, "",'The answer is `$(Get-Process lsass).Id.ToString()`, run it in your terminal.'),
    ...
    [Question]::new(2, 'default', 'The ''CommandType'' field (not the value) in the output of the command ''Get-Command Invoke-Expression'' is called a Member. What is the ''MemberType'' of the ''CommandType'' member? ', $null, "Property", 'MemberType is visible with the Get-Member command. Here is an example on how to use Get-Member `Get-Process -Name "chrome" | Get-Member` to view all of the output''s members.'),
    [Question]::new(3, 'default', 'What is the cmdlet behind the alias of the command "ls"', $null, 'get-childitem','To view the cmdlet behind an alias, use Get-Alias'),
    [Question]::new(4, 'code', 'How many services currently exist on your machine (in all states)?', {$(Get-service).count},'', 'To nicely count items you wrap the command output and treat it as an object like this `$(<command>).count`, or to use ` | Measure-Object`'),
    ...
    [Question]::new(6, 'code', 'How many available commands in your machine has "Microsoft.PowerShell.Utility" as ther ModuleName (No Aliases!)?', { $(Get-command | Where-Object{$_.ModuleName -eq "Microsoft.PowerShell.Utility"}).Count },'' ,"It will probably be best to add ``| Where-object <add some stuff here>`` to the Get-Command output. To view the ModuleName and the rest of the properties use ``| Select-Object *`` or ``| Format-List *``."),
    [Question]::new(7, 'default', 'What is the type of the output to the command "Get-Help cat" ', $null , 'PSCustomObject', 'Wrapping output with $(<Command>) can make your life easier. To view available action, use Get-Member.'), #{ $(get-help cat ).gettype().name }
    [Question]::new(8, 'code', 'What is the 77th char of the output to the command "Get-Help cat -full" ', { $(get-help cat -full | out-string)[77] }, '', 'First you need to convert to string, sometimes there multiple ways to achieve the same result.'),
    ...
    [Question]::new(10, 'code', 'What is the character length of the Subject name who issued the digital certificate for the file "C:\windows\system32\ntoskrnl.exe"? ', { $(Get-AuthenticodeSignature "C:\windows\system32\ntoskrnl.exe").SignerCertificate.Subject.trim().Length }, '', "Did you look within the SignerCertificate member? Use Get-AuthenticodeSignature. Using Select-Object or wrapping the output with `$() can help."),
